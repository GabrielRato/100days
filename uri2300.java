...
// getters and setters
    ...
    public static class Graph {
        ...
        public boolean isConnected(int qtdNodes) {
            ...
            nodes.forEach(node -> {
                qtd.addAndGet(canReachAllNodes(node, qtdNodes) ? 1 : 0);
            });
        ...
        while (e != 0) {
            ...
            for (int i = 0; i < e; i++) {
                ...
                if (graph.nameToIdx.get(source) != null) {
                    nodeSource = graph.nodes.get(graph.nameToIdx.get(source));
                } else {
                    nodeSource = new Node(source);
                }

                //check also if dest already exist
                if (graph.nameToIdx.get(destination) != null) {
                    nodeDestination = graph.nodes.get(graph.nameToIdx.get(destination));
                } else {
